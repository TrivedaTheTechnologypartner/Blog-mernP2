...
import serviceAccountKey from "./react-js-blog-website-yt-ded86-firebase-adminsdk-lmrz6-e9d71ed613.json" assert {type : 'json'}
...
server.post("/signin", (req,res) => {
    ...
    .then((user) => {
        ...
        if(!user.google_auth){            bcrypt.compare(password, user.personal_info.password, (err, result) => {
...
server.post('/create-blog', verifyJWT, (req,res) => {
    ...
    if(!title.length){
        return res.status(403).json({error : "You must provide title to publish the blog "})
    }
